...
### [ ] P5-1 决策理由链 (Decision Rationale Chain)

**目标**：每个关键决策点生成结构化理由，包含 evidence + counterfactual + timestamp
**新增文件**：`agents/orchestrator/rationale.py`
**修改**：`agent_graph.py` 每个节点返回 `rationale` 字段

---

### [ ] P5-2 反事实推理 (Counterfactual Reasoning)
...
### [ ] P5-3 不确定性量化 (Uncertainty Quantification)

**目标**：每个预测像素输出数据不确定性 + 模型不确定性 + 90% CI
**基础**：已有 `committee_agreement.png`（委员会一致性）→ 升级为完整 UQ
**修改**：`agent_graph.py` — `_predict_ensemble` 扩展

---

## 🎯 P6 — 主动学习 (Active Learning)

### [ ] P6-1 环境空间覆盖率分析

**目标**：建模完成后输出"知识缺口地图"和"不确定性热力图"
...
**目标**：自动生成带优先级排序的建议采样点列表
**输出**："建议在 (119.5°E, 34.2°N) 采样，可预期 AUC 提升 0.05-0.08"

---

### [ ] P6-3 贝叶斯优化迭代循环

**目标**：数据补充 → 重训练 → 对比前后变化的完整闭环

---

## 🔗 P7 — 多模态数据融合

### [ ] P7-1 文献知识提取 (Literature → Prior)

**目标**：从论文摘要自动提取物种生态学参数，转化为贝叶斯先验
**新增目录**：`agents/knowledge_extractor/`
**依赖**：Semantic Scholar API（免费）

---

### [ ] P7-2 时间序列注意力 (Temporal Attention)

**目标**：用 12 月序列替代年均值，Transformer 学习季节性权重
**新增目录**：`agents/temporal/`
**依赖**：PyTorch（可选）
...
**目标**：从 Landsat/Sentinel-2 提取珊瑚礁/海草床/海岸线距离等生境结构
...
### [ ] P8-1 质疑者智能体 (Devil's Advocate Agent)

**目标**：专门检验模型假设（多重共线性、切分策略、伪缺失合理性）
**新增目录**：`agents/debate/`
...
**目标**：每个关键决策经历一轮辩论，输出"辩论摘要"
**新增文件**：`agents/debate/arbiter.py`
...
**目标**：自动运行 2000-2010 vs 2010-2020 对比，检测适宜栖息地重心位移

---

### [ ] P9-2 生态位漂移检测

**目标**：自动报警"检测到物种北移 1.2°/10年"

---

### [ ] P9-3 CMIP6 未来投影

**目标**：对接 ssp126/ssp370/ssp585，输出 2050/2070/2090 分布 + 迁移走廊
...
**目标**：海洋保护区规划 / 入侵物种预警 / 气候变化适应 三个场景模板
**输出**：PDF 报告而非原始指标
...
**目标**：Streamlit 无代码界面 + Docker 一键部署 + 5 个示例数据集

---

### [ ] P10-3 模型持续学习 (Online Learning)
...
```
已完成 ✅                        2-4周 (路径A)                   1-2月 (路径B)                 3-6月 (路径C)
┌──────────────┐    ┌─────────────────────────┐    ┌──────────────────────┐    ┌─────────────────────┐
│P0-2 Agent图  │    │P5-1 决策理由链          │    │P6-1,2 主动学习       │    │P8 科学辩论          │
│P1-1 4算法    │ →  │P5-2 反事实推理          │ →  │P7-1 文献知识提取     │ →  │P9-2 生态位漂移      │
│P1-2 biomod2  │    │P5-3 不确定性量化        │    │P7-2 时间序列注意力   │    │P10-1 场景化报告     │
│P1-3 可解释性 │    │→ 投稿 MEE (路径A)       │    │P9-3 CMIP6 投影       │    │P10-2 无代码界面     │
│P1-4 Ensemble │    │                         │    │→ 投稿 NC/PANS (路径B)│    │→ 投稿 SA/NEE (路径C)│
└──────────────┘    └─────────────────────────┘    └──────────────────────┘    └─────────────────────┘
```
